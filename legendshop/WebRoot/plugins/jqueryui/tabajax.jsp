...
<link rel="stylesheet"
	href="<ls:templateResource item='/plugins/jqueryui/css/jquery.ui.all.css'/>">
<script src="<ls:templateResource item='/common/js/jquery.js'/>"
	type="text/javascript"></script>
<script
	src="<ls:templateResource item='/plugins/jqueryui/js/jquery.ui.core.js'/>"
	type="text/javascript"></script>
<script
	src="<ls:templateResource item='/plugins/jqueryui/js/jquery.ui.widget.js'/>"
	type="text/javascript"></script>
<script
	src="<ls:templateResource item='/plugins/jqueryui/js/jquery.ui.tabs.js'/>"
	type="text/javascript"></script>
<link rel="stylesheet"
	href="<ls:templateResource item='/plugins/jqueryui/css/demos.css'/>">
		...
		<thead>
			<tr>
				<td><a href="<ls:url address='/admin/index'/>" target="_parent">首页</a>
					&raquo; 商品管理 &raquo; <a
					href="${pageContext.request.contextPath}/admin/order/processing">订单管理</a></td>
			</tr>
			<tr>
				<td><a href="javascript:void(0)" onclick="changetab(1);"
					id="processingbutton" name="processingbutton">处理中的订单</a> <a
					href="javascript:void(0)" onclick="changetab(2);"
					id="processedbutton" name="processedbutton"><span>已成交订单</span></a>
				</td>
			</tr>
		</thead>
		<tbody>
			<tr>
				<td>
					<form
						action="${pageContext.request.contextPath}/admin/order/processing"
						id="processedOrderForm" name="processedOrderForm" method="post"
						style="margin: 0px">
						&nbsp; <input type="hidden" id="curPageNO" name="curPageNO"
							value="${curPageNO}" />
						<div align="left" style="padding: 3px">
							<input type="hidden" name="subCheck" id="subCheck" value="N" />
							&nbsp; 订单号&nbsp;<input type="text" name="subNumber"
								id="subNumber" value="${subForm.subNumber}" size="30"
								maxlength="30" /> &nbsp; 买家&nbsp;<input type="text"
								name="userName" id="userName" value="${subForm.userName}" />
							<auth:auth ifAnyGranted="F_VIEW_ALL_DATA">
               &nbsp; 商城&nbsp;<input type="text" name="shopName"
									id="shopName" value="${subForm.shopName}" />
							</auth:auth>
							&nbsp; 状态&nbsp; <select id="status" name="status" class="input">
								<ls:optionGroup type="select" required="false" cache="true"
									beanName="ORDER_STATUS" selectedValue="${subForm.status}" />
							</select> <input type="submit" value="搜索" class="s" />
					</form>
					</div>
				</td>
			</tr>
	...
	<div class="demo">
		...
		<div id="tabs">
			...
			<div id="tabs-1">
						...
						<td valign="top">
							<div align="center">
								...
								<display:table name="list"
									requestURI="${pageContext.request.contextPath}/admin/order/processing"
									id="item" export="true" class="${tableclass}"
									style="width:100%" sort="external">
									<display:column title="顺序" class="orderwidth"><%=offset++%></display:column>
									<display:column title="订单号" sortable="true"
										sortName="subNumber">
										<a
											href='${pageContext.request.contextPath}/p/orderDetail/${item.subNumber}'
											target="_blank">${item.subNumber}</a>
									</display:column>
									<display:column title="价格" sortable="true" sortName="total">
										<b><fmt:formatNumber type="currency" value="${item.total}"
												pattern="${CURRENCY_PATTERN}" /></b>
									</display:column>
									<display:column title="商品" property="prodName" sortable="true"
										sortName="prodName"></display:column>
									<display:column title="买家帐号" sortable="true"
										sortName="userName" style="width: 80px">${item.userName}</display:column>
									...
									<auth:auth ifAnyGranted="F_VIEW_ALL_DATA">
										<display:column title="商城" property="shopName" sortable="true"
											sortName="shopName">
											<a
												href="${pageContext.request.contextPath}/shop/${item.shopName}"
												target="_blank">${item.shopName}</a>
										</display:column>
									</auth:auth>
									...
									<auth:auth ifAnyGranted="F_SYSTEM">
										<display:column title="操作" media="html" style="width:110px">
											<a
												href='javascript:orderList.orderDetail("${item.subNumber}");'>详情</a>
											<c:if test="${subForm.subCheck == 'N'}">
												<c:if test="${item.status == 1}">
													<!-- 1: 等待买家付款 -->
													<a href="javascript:void(0)"
														onclick='javascript:openScript("${pageContext.request.contextPath}/admin/order/modifyPrice?total=${item.total}&subId=${item.subId}&subNumber=${item.subNumber}","330","150")'>修改</a>
												</c:if>
												<c:if test="${item.status == 2}">
													<!-- 2:买家已经付款 -->
													<!-- 4:交易成功 -->
													<a href="javascript:void(0)"
														onclick='javascript:updateSubStatus("${item.subId}",3,"${item.subNumber}");'>发货</a>
												</c:if>
												<c:if
													test="${item.payTypeId == 3 &&( item.status != 3 && item.status != 6)}">
													<!-- 3:货到付款,没有退货又没有发货的 -->
													<a href="javascript:void(0)"
														onclick='javascript:updateSubStatus("${item.subId}",3,"${item.subNumber}");'>发货</a>
												</c:if>
											...
											<auth:auth ifAnyGranted="F_VIEW_ALL_DATA">
												<a href="javascript:void(0)"
													onclick='javascript:deleteSub("${item.subId}","${item.subNumber}");'>删除</a>
											</auth:auth>
										</display:column>
									</auth:auth>
								</display:table>
				...
				说明：<br> 1. 用户支付之后需要点击发货按钮，用户确认收货，整个订单完成，进入历史订单状态<br> 2.
				用户申请退款，在退款之后点击关闭交易，订单完成。<br>
   ...
   function pager(curPageNO){
	...
	<div class="demo-description">
		...
		<p>Tabs 3 and 4 demonstrate slow-loading and broken AJAX tabs, and
			how to handle serverside errors in those cases. Note: These two
			require a webserver to interpret PHP. They won't work from the
			filesystem.</p>
	</div>
