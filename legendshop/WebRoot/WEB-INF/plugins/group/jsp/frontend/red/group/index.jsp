...
<body>
	...
	<div class="w banner1">
		<c:forEach items="${requestScope.INDEX_ADV_1}" var="adv">
			<table width="1216px" cellpadding="0" cellspacing="0"
				style="margin-top: 5px" class="picstyle">
				<tr>
	...
	<div class="w">
		...
		<!----左边---->
		<div class="t_left">
			<form action="<ls:url address='/group/index'/>" method="get"
				id="form1">
				<input type="hidden" id="curPageNO" name="curPageNO"
					value="<%=request.getAttribute("curPageNO")%>"> <input
					type="hidden" id="sortId" name="sortId" value="${prod.sortId }" />
					<input type="hidden" id="order" name="order" value="${order }" />
					<input type="hidden" id="seq" name="seq" value="${seq}" />
			</form>
			<div id="filter">
				<ul class="cf">
					<li class="first <c:if test='${empty prod.sortId }'>current</c:if>"><a
						href="<ls:url address='/group/index'/>">全部分类</a></li>

					<c:forEach items="${groupSortList }" var="item">
						<li
							class="<c:if test='${prod.sortId eq item.sortId }'>current</c:if>"><a
							href="javascript:changeCate(${item.sortId})">${item.sortName }<!-- <span>(33)</span> --></a></li>
					</c:forEach>
			...
			<div class="along">
				<div class="aligna">
					<span class="fl">排序方式：</span> <a
						href="javascript:changeOrder('recommend')"
						class="<c:if test="${empty order or order eq 'recommend' }">aligna_aon</c:if>">推荐</a>
					<a href="javascript:changeOrder('hot')"
						class="<c:if test="${order eq 'hot' }">aligna_aon</c:if>">人气</a>
					...
					<a href="javascript:changeOrder('time')"
						class="<c:if test="${order eq 'time' }">aligna_aon</c:if>">即将过期</a>
				</div>
			...
			<c:forEach items="${list }" var="item" varStatus="vs">
				...
				<div class="list_item <c:if test='${vs.count % 2 eq 0 }'>odd</c:if>">
					<h1>
						<a href="<ls:url address='/group/view/${item.prodId }'/>"> <c:choose>
					...
					<div class="cover">
						<a href="<ls:url address='/group/view/${item.prodId}'/>"><img
							src="<ls:photo item='${item.pic}'/>" width="372" height="225" /></a>
					</div>

					<div class="priceit">
						<span><font>${item.buys }</font>人已购买</span> 原价：<font><fmt:formatNumber
								value="${item.price}" pattern="#.0#" />元</font> <font class="zhe">
							<c:if test="${item.cash > 0 and item.price > 0}">
								<fmt:formatNumber value="${item.cash*10 / item.price}"
									pattern="#.#" />折
                        </c:if>
						</font>
					</div>
					<a title="团购" href="<ls:url address='/group/view/${item.prodId}'/>">
						<div class="buyit">
							<span><fmt:formatNumber value="${item.cash}"
									pattern="#.0#" /></span> <span class="tg"></span>
						</div>
					</a>
				</div>
