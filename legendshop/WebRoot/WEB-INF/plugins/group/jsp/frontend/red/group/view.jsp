...
<link rel="stylesheet" type="text/css" media='screen'
	href="<ls:templateResource item='/common/default/css/addtocart.css'/>" />
...
//加入购物车
 function showcart(show){
	...
	if(show == 0){
		...
		var prodAttr = "";
		   CommonService.addtocart(${product.prodId}, count, prodAttr, function(retData){
...
<div class="w">
	...
	<div class="t_left">
		...
		<div class="t_info_box">
				...
				<table class="deal_discount">
					<tbody>
						...
						<tr>
							...
							<th><c:if test="${product.cash > 0 and product.price > 0 }">
									<span><fmt:formatNumber
											value="${product.cash*10 / product.price}" pattern="#.#" />折</span>
								</c:if></th>
							<th><span>¥<fmt:formatNumber
										value="${product.price-product.cash}" pattern="#.0#" /></span></th>
						</tr>
					</tbody>
				</table>
				<div class="deal-buy">
					<p class="deal-price">
						<strong>¥<fmt:formatNumber value="${product.cash}"
								pattern="#.0#" /></strong><span><a href="javascript:showcart(1)">抢购</a></span>
					</p>
				</div>

				<div class="downinfo" style="margin-top: 75px;">
					<p>
						<span class="text_red">${product.buys+visualBuys }</span> 人已购买
					</p>
				</div>

				<div class="downinfo">
					<p>
						<label>本团购剩余</label>${product.actualStocks  }<br />
						<c:if test="${not empty  product.endDate }">
							<div class="lxftime"
								endtime="<fmt:formatDate value="${product.endDate }" pattern="MM/dd/yyyy hh:mm:ss" />"></div>
						</c:if>
			...
			<div class="side">
				<div class="deal-buy-cover-img">
					<img src="<ls:photo item='${product.pic}'/>" width="440">
				</div>
			</div>
			<div class="clear"></div>
		...
		<!----up end---->
		<div id="facebox"
			style="position: absolute; z-index: 9999; top: 260px; left: 430px; display: none; overflow: hidden;">
			<div>
				<h2>
					<span id="can_AddToCart"><fmt:message
							key="add.tocart.seccess" /></span>
				</h2>
				<p>
					<fmt:message key="cart.have.products">
						<fmt:param value='<span id="basket_count">1</span>' />
					</fmt:message>
					<span id="basket_total_cash">1.00</span>&nbsp;
					<fmt:message key="curreny.unit" />
				</p>
				<p style="color: #666">
					<span id="go_ToCash" style="display: none;"><input
						type="button" value='<fmt:message key="goto.settlement" />'
						onclick="javascript:gotoCash();" /></span> <a
						href="javascript:showcart(0)"><fmt:message key="buy.others" /></a>
					<span id="product_Less" style="display: none;"><a
						href="${pageContext.request.contextPath}/leaveword"><fmt:message
								key="notice.product.less" /></a></span>
				</p>
			</div>
		</div>
		...
		<div class="t_info_box t_inback">
			<div class="infonav">
				<table width="425" height="41">
					<tr>
						<td width="70" align="center"><strong>商品详情</strong></td>
						<td width="3" align="center"><img
							src="${pageContext.request.contextPath}/img/group/bg2_06.gif"
							width="3" height="32" /></td>
             ...
             <td width="52" align="center" class="text_red">$48</td> --%>
						<td align="center">
							<%-- <img src="${pageContext.request.contextPath}/img/group/gobuy.gif" width="87" height="29" /> --%>
						</td>
					</tr>
				</table>
			</div>
			<div class="tinfo_left">${product.content }</div>
			...
			<div class="tinfo_right">
				<c:if test="${not empty partner }">
					...
					<c:if test="${not empty partner.homepage }">
						<strong>主页：</strong>
						<a href="${partner.homepage}">${partner.homepage }</a>
						<br />
					</c:if>
...
<!----两栏end---->
<form action="${pageContext.request.contextPath}/basket/save" id="form1"
	method="post">
