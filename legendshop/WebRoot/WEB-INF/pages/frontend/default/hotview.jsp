...
<![endif]-->
	<div class="tables">
		<table class="recommond" width="100%" cellpadding="0" cellspacing="0">
			<tr>
				<td><fmt:message key="shop.hot.product">
						<fmt:param value="<lb:currentShop />" />
					</fmt:message></td>
			</tr>
			<tr>
				<td><table style="font-weight: normal; color: #666666">
						<tr>
							<c:forEach items="${requestScope.hotViewList}" var="prod"
								varStatus="status">
								<%-- 
	...
	--%>
								<td width="20%" align="center"><c:choose>
										<c:when test="${fn:length(prod.name) > 30}">
											<span> <a
												href="${pageContext.request.contextPath}/views/${prod.prodId}"><img
													src="<ls:images item='${prod.pic}' scale='1'/>"
													width="150px" height="150px" title="${prod.name}" id="pic"></a></span>
										...
										<c:otherwise>
											<span> <a
												href="${pageContext.request.contextPath}/views/${prod.prodId}"><img
													src="<ls:images item='${prod.pic}' scale='1'/>"
													width="150px" height="150px" id="pic"></a></span>
											<br>${prod.name}<br>
