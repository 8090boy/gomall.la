				...
				<table cellpadding="2" cellspacing="2">
					<tr>
						<c:forEach items="${requestScope.externalLinkList}" var="el">
							<td height="40px" align="center"><a href="${el.url}"
								target=_blank title="${el.content}"> <c:choose>
										<c:when test="${el.picture != null}">
											<img src="<ls:photo item='${el.picture}'/>"
												title="${el.wordlink}" width="88px" height="31px" />
											<br>${el.wordlink}
      			</c:when>
...
<c:choose>
	<c:when test="${shopDetail != null}">
		<table width="954px" class="tables" style="margin-top: 5px"
			cellpadding="0" cellspacing="0">
			<tr>
				<td class="titlebg"><fmt:message key="order.step" /></td>
			</tr>
			<tr>
				<td><jsp:include page="/copy" /></td>
			</tr>
		</table>
	</c:when>
	<c:otherwise>
