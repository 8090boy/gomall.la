...
<table width="100%" class="tables" cellpadding="0" cellspacing="0">
	<tr>
		<td class="titlebg"><fmt:message key="newsCenter" /></td>
	</tr>
	<ls:cache key="ALL_NEWS_CATEGORY_${newsCategoryId}"
		cacheName="NewsList">
		<tr>
			<td>
				<div style="margin: 8px;" align="right">
					<form action="${pageContext.request.contextPath}/allnews"
						...
						： <select id="newsCategoryId" name="newsCategoryId"
							...
							<ls:optionGroup type="select" required="true" cache="fasle"
								...
								param="${sessionScope.shopName}"
								selectedValue="${newsCategoryId}" />
						</select> <input type="hidden" id="curPageNO" name="curPageNO"
							value="${curPageNO}" />
	...
	<c:forEach items="${requestScope.list}" var="news" varStatus="status">
		<tr>
			<td align="left"
				style="border-bottom: 1px dotted #E9E9E9; padding: 3px">
				<div>
					&nbsp;${status.index+1}. <a
						href="${pageContext.request.contextPath}/news/${news.newsId}">
						<c:choose>
