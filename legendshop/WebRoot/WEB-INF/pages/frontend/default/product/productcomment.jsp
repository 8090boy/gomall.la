...
<c:if test="${requestScope.productCommentList != null }">
	<table width="100%" class="tables" cellpadding="0" cellspacing="0">
		...
		<tr>
			<c:forEach items="${requestScope.productCommentList}" var="comment"
				varStatus="status">
				<tr>
					<td align="left" style="width: 80%">
						<div>
							<span style="color: red">&nbsp;[<fmt:formatDate
									value="${comment.addtime}" pattern="yyyy-MM-dd HH:mm" />]
							</span> &nbsp; ${comment.content}
						</div> <c:if test="${comment.replyContent != null}">
	...
	<tr>
		<td>
			<table style="width: 100%">
				<tr>
					<td width="150px" align="right"><font color="#ff0000">*</font>
						<fmt:message key="content" /></td>
					<td align="left"><textarea rows="5" cols="50" id="content"
							name="content" style="font-size: 10pt;"></textarea></td>
				</tr>
				<tr>
						...
						<div align="center">
							...
							<c:choose>
								<c:when
									...
									<input type="button" value='<fmt:message key="logon.first"/>'
										class="s" disabled="disabled">
								</c:when>
								<c:otherwise>
									<input type="button" value="<fmt:message key="submit"/>"
										class="s" onclick="addProdcutComment();" />
								</c:otherwise>
							</c:choose>
							<% }else{%>
							<input type="button" value="<fmt:message key="submit"/>"
