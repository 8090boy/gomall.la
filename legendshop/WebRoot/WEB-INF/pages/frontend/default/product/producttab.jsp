...
<link rel="stylesheet" type="text/css" media='screen'
	href="<ls:templateResource item='/common/default/css/productTab.css'/>" />
<script type="text/javascript" language="javascript"
	src="<ls:templateResource item='/common/default/js/productTab.js'/>"></script>
<DIV id="con">
	<UL id="tags">
		<LI class="selectTag"><A onClick="selectTag('tagContent0',this)"
			href="javascript:void(0)"><fmt:message key="prod_content" /> </A></LI>
		<LI><A onClick="selectTag('tagContent1',this)"
			href="javascript:void(0)"><fmt:message key="product.parameter" />
		</A></LI>
		<LI><A onClick="selectTag('tagContent2',this)"
			href="javascript:void(0)"><fmt:message key="product.comment" />
		</A></LI>
		<c:if
			test="${requestScope.productList != null && fn:length(requestScope.productList) > 0}">
			<LI><A onClick="selectTag('tagContent3',this)"
				href="javascript:void(0)"><fmt:message key="related.products" />
			</A></LI>
		</c:if>
	</UL>
	<DIV id="tagContent">
		<DIV class="tagContent selectTag" id="tagContent0">
			${prod.content} <br></br> ${shopDetail.detailDesc} <br />
		</DIV>
		<DIV class="tagContent" id="tagContent1">
			<div id="prodParameter" name="prodParameter"></div>
			<br /> <br /> ${shopDetail.detailDesc} <br />
		</DIV>
				...
				<table width=100% " cellpadding="0" cellspacing="10" align="left">
					<tr>
						<c:forEach items="${requestScope.productList}" var="prod"
							varStatus="status">
							<td width="33%" align="left"><c:choose>
									<c:when test="${fn:length(prod.name) > 30}">
										<span> <a
											href="<ls:url address='/views/${prod.prodId}'/>"> <img
												src="<ls:images item='${prod.pic}' scale='1'/>"
												width="150px" height="150px" title="${prod.name}" id="pic">
										</a>
										</span>
										<br>${fn:substring(prod.name,0,30)}...<br>
									...
									<c:otherwise>
										<span> <a
											href="<ls:url address='/views/${prod.prodId}'/>"> <img
												src="<ls:images item='${prod.pic}' scale='1'/>"
												width="150px" height="150px" id="pic">
										</a>
										</span>
										<br>${prod.name}<br>
									</c:otherwise>
								</c:choose> <c:if test="${not empty prod.cash}">
