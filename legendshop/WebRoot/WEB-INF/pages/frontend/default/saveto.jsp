...
<c:choose>
	...
	<c:otherwise>
		<table width="100%" cellspacing="0" cellpadding="0" align="center"
			style="margin-top: 5px;" class="tables">
			<tr>
				...
				<td><c:forEach items="${requestScope.subList}" var="sub">
						<form action="${pageContext.request.contextPath}/payment/payto"
							...
							<input type="hidden" name="subNumber" id="subNumber"
								...
								name="aliorder" id="aliorder" value="${sub.name}" /> <input
								type="hidden" name="alibody" id="alibody"
								value="${member.other}" /> <input type="hidden" name="alimoney"
								id="alimoney" value="${sub.total}" /> <input type="hidden"
								name="shopName" id="shopName" value="${sub.shopName}" /> <input
								type="hidden" name="payTypeId" id="payTypeId" value="3" />
							<!-- default 货到付款 -->
									...
									<td>
										<div
											...
											：<span style="color: red; font-weight: bold;"><a
												href="<ls:domain shopName='${sub.shopName}' />">${sub.shopName}</a></span>
											&nbsp;
									...
									<td>
										<table width="100%">
											<tr>
												...
												<td width="30px"><fmt:message key="product.number" /></td>
												<td><fmt:message key="price.hint" /></td>
												<td>&nbsp;</td>
											</tr>
												...
												<tr>
													...
													<td><a target="_blank"
														href="${pageContext.request.contextPath}/views/${basket.prodId}"><font
															color="#FF0000">${basket.name}</font></a></td>
													<td>${basket.basketCount}</td>
													<td><fmt:formatNumber type="currency"
															value="${basket.cash}" pattern="${CURRENCY_PATTERN}" /> <c:if
															test="${basket.carriage != null}">
                  					(<fmt:message key="carriage.charge" />
															<fmt:formatNumber type="currency"
																value="${basket.carriage}" pattern="${CURRENCY_PATTERN}" />)</c:if>&nbsp;&nbsp;</td>
								...
								<tr align="left">
									<td>
										<table style="margin-left: 30px">
											<tr height="30px">
												<td><fmt:message key="total.charge" /> <span
													style="color: red; font-weight: bold; font-size: 12pt"><fmt:formatNumber
															type="currency" value="${sub.total}"
															pattern="${CURRENCY_PATTERN}" /></span> &nbsp; <c:if
														test="${sub.score ne null}">
														<fmt:message key="score.use" />：${sub.score}</c:if> <%
        	  		if(PropertiesUtil.getObject(SysParameterEnum.USE_SCORE, Boolean.class)){
        	  	%> <c:if
														test="${sub.scoreId == null and (sub.status == 1 or sub.status == 7)}">
														<fmt:message key="score.add" />：<font
															style="color: red; font-weight: bold; font-size: 10pt"><fmt:formatNumber
																value="${sub.total}" pattern="#" type="number" /></font> &nbsp; 
        	  	</c:if> <c:if test="${availableScore > 0 && sub.scoreId == null}">
														<fmt:message key="available.score" />：<font
															style="color: red; font-weight: bold; font-size: 10pt">${availableScore}</font> &nbsp;
	        	  	<a href='javascript:void(0)'
															onclick='javascript:useScore("${sub.subNumber}","${sub.subId}","${sub.total}")'><fmt:message
																key="score.buy" /></a> &nbsp;
        	  	</c:if> <%} %></td>
											</tr>
											<c:if test="${sub.status == 1 or sub.status == 7}">
												<!-- 不是货到付款，等待客户付款状态 -->
												<tr height="40px">
													<td><fmt:message key="payType" /> <c:forEach
															items="${sub.payType}" var="payTypeInstance">
															<input type="button"
																value="${payTypeInstance.payTypeName}"
																id="${payTypeInstance.payTypeId}" class="s"
																onclick="payto(this)" />
														</c:forEach></td>
												</tr>
											</c:if>
									...
									<tr>
										<td>
											<p
												...
												<font color="#FF9900"><fmt:message key="after.order" /></font>
										</td>
									</tr>
								</table>
							</td>
...
<script type="text/javascript">
	function payto(obj){
		...
		payForm["payTypeId"].value = obj.id;
	 	$.post("${pageContext.request.contextPath}/p/payToOrder" , 
	    {"subId": subId, "shopName": payForm["shopName"].value,"payTypeId": payForm["payTypeId"].value},
        function(retData) {
		},'json');
		if(obj.id != 3){
			payForm.submit();
	...
	function updateSubStatus(subId,status) {
		 var data ={"subNumber": subNumber, "status": status};
		    jQuery.ajax({
