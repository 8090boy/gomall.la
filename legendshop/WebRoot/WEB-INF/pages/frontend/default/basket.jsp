...
<![endif]-->
<script
	src="${pageContext.request.contextPath}/common/js/jquery.tools.min.js"></script>
<link rel="stylesheet" type="text/css" media='screen'
	href="${pageContext.request.contextPath}/common/default/css/overlay.css" />
<link rel="stylesheet"
	href="${pageContext.request.contextPath}/common/default/css/jquery.superbox.css"
	type="text/css" media="all" />
<script type="text/javascript"
	src="${pageContext.request.contextPath}/common/js/jquery.superbox-min.js"></script>
<script
...
<c:if test="${sessionScope.SPRING_SECURITY_LAST_USERNAME != null}">
	<form id="form1" method="post" action="buy">
		<input name="action" value="buy" type="hidden" /> <input name="prodId"
			value="${prod.prodId}" type="hidden" /> <input name="pic"
			value="${prod.pic}" type="hidden" /> <input name="name"
			value="${prod.name}" type="hidden" /> <input name="cash"
			value="${prod.cash}" type="hidden" />
		<table width="100%" cellspacing="0" cellpadding="0" align="center"
						...
						<tr>
							<td valign="top" align="center" width="256px">
								<div id="apple">
									<img src="<ls:images item='${prod.pic}' scale='1'/>"
										width="150px" height="150px" style="margin-right: 10px"
										rel="#${prod.prodId}" />
								</div>
								<div class="apple_overlay" id="${prod.prodId}">
									<img src="<ls:photo item='${prod.pic}'/>" width="640px"
										height="470px" />
									<div class="details">
										<h2>${prod.name}</h2>
									</div>
							...
							<td align="center" valign="top">
								...
								<table width="100%" border="1" cellpadding="0" cellspacing="0"
									style="border-collapse: collapse; margin-top: 7px; border: 1px solid #CCCCCC;"
									bgcolor="#ECECEC">
									<tr align="center">
										<td height="30" colspan="2" style="margin-left: 10px"><b><a
												href="${pageContext.request.contextPath}/views/${prod.prodId}">${prod.name}</a></b></td>
									</tr>
									<tr bgcolor="#FFFFFF">
											...
											<tr bgcolor="#FFFFFF">
												...
												<td>
													<div align="left" class="prodattr">
														...
														<font color="#D03430"><fmt:formatNumber
																type="currency" value="${prod.cash}"
																pattern="${CURRENCY_PATTERN}" /></font>
														<c:if test="${prod.carriage != null}">
                                  &nbsp;&nbsp;<fmt:message
																key="carriage.charge" />
															<font color="#D03430"><fmt:formatNumber
																	type="currency" value="${prod.carriage}"
																	pattern="${CURRENCY_PATTERN}" /></font>
															<input type="hidden" id="carriage" name="carriage"
																value="${prod.carriage}" />
														</c:if>
													</div>
												</td>
											...
											<c:if test="${not empty prod.cash}">
												<tr bgcolor="#FFFFFF">
													...
													<td>
														<div align="left" class="prodattr">
															<font color="#D03430"><b><span id="totalFee"><fmt:formatNumber
																			type="currency" value="${prodTotalFee}"
																			pattern="${CURRENCY_PATTERN}" /></span></b></font>
														</div>
													</td>
												</tr>
											</c:if>
													...
													<td>
														<div align="left" class="prodattr"
															...
															<input type="hidden" id="attribute" name="attribute"
																value="${sessionScope.BASKET_HW_ATTR}" />
											...
											<c:if test="${not empty prod.cash}">
												<tr>
													<td height="29">
														<div align="left" class="prodattr">
															<fmt:message key="comfirm.order" />
														</div>
													</td>
													<td>
														<div align="left" class="prodattr">
															<c:if test="${sessionScope.BASKET_HW_COUNT != null}">
																<input type="text" class="input2" id="count"
																	name="count" maxlength="8" size="3"
																	value="${sessionScope.BASKET_HW_COUNT}"></input>
															</c:if>
															<c:if test="${sessionScope.BASKET_HW_COUNT == null}">
																<input type="text" class="input2" id="count"
																	name="count" maxlength="8" size="3" value="1"></input>
															</c:if>

															<input type="button" value="<fmt:message key="submit"/>"
																class="s" onclick="javascript:basket()" />
														</div>
													</td>
												</tr>
											</c:if>
			...
			<tr>
				<td><jsp:include flush="true" page="/p/bought" /></td>
			</tr>
		</table>
	</form>
</c:if>
