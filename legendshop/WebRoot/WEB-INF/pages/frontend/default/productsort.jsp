...
<table cellspacing="0" cellpadding="0" width="744px">
	<tr>
		<c:choose>
			<c:when test="${sort.picture !=null}">
				<td><img src="<ls:photo item='${sort.picture}'/>" width="740px"
					height="240px" class="banner"></a></td>
			</c:when>
			<c:otherwise>
	...
	<c:if test="${sort.sortName != null}">
		<tr>
			<td
				style="border-bottom: #cccccc 1px solid; background-color: #f7f7f7"
				width="100%">
				<table width="744px">
					<tr>
						<td height="25" align="left">${sort.sortName} <c:if
								test="${requestScope.hotProdList!=null}">&nbsp; <fmt:message
									key="hot.product" />：
				<c:forEach items="${requestScope.hotProdList}" var="search">
						...
						<td align="right"><c:if
								test="${not empty requestScope.nsortList}">
							...
							<form id="PagingForm" method="post"
								action="${pageContext.request.contextPath}/nsort">
								<input type="hidden" id="sortId" name="sortId"
									value="${sort.sortId}"> <input type="hidden"
									id="nsortId" name="nsortId">
							</form></td>
					</tr>
				</table>
			</td>
	...
	<tr>
		<td>
			<table>
				<tr>
					<td valign="top">
						<form action="${pageContext.request.contextPath}/sort" id="form1"
							method="post">
							<input type="hidden" id="curPageNO" name="curPageNO"
								value="${prod.curPageNO}" /> <input type="hidden" name="sortId"
								value="${sort.sortId}">
							<table width="100%" cellspacing="0" cellpadding="0">
															...
															<td align="center">
																<div id="apple">
																	<a
																		...
																		<img
																		src="<ls:images item='${prodDetail.pic}' scale='1'/>"
																		width="150px" height="150px" style="margin: 2px">
																	</a>
																</div>
															</td>
														</tr>
														<tr align="center">
															<td width="100%" align="center"
																...
																<div class="topnewsfixed" title="${prod.name}">
																	<a
																		href="${pageContext.request.contextPath}/views/${prodDetail.prodId}">${prodDetail.name}</a>
																</div>
															</td>
														</tr>
														<tr>
															<td align="center" valign="top"><c:if
																	test="${not empty prodDetail.cash}">
								...
								<tr>
									...
									<c:otherwise>
										<td align="center"><fmt:message key="nothingfound" /></td>
									</c:otherwise>
									</c:choose>
								</tr>
							</table>
