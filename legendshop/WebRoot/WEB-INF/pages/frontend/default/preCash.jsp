		...
		<tbody>
			<c:forEach items="${requestScope.baskets}" var="basket">
				<tr class="item-content">
					<!-- 商品 -->
					<td><input type="hidden" name="strArray"
						...
						<div class="item-pic J_ItemPic img-loaded">
							<a target="_blank"
								...
								src="<ls:images item='${basket.pic}'  scale='2'/>" width="100px"
								height="100px" />
							</a>
					...
					<!-- 属性 -->
					<td>积分：${basket.points}<br />${basket.attribute}</div>
					</td>
					<!-- 数量 -->
					<td>x ${basket.basketCount}<input type="hidden"
						id="basketCount${basket.basketId}"
						name="basketCount${basket.basketId}" value="${basket.basketCount}"></td>
					<!-- 金额 -->
					<td class="td-sum">
						<div id="currencyLabel">
							<fmt:formatNumber type="currency" value="${basket.total}"
								pattern="${CURRENCY_PATTERN}" />
						</div> <c:if test="${basket.carriage != null && basket.carriage > 0}">
							<fmt:message key="carriage.charge" />
							<fmt:formatNumber type="currency" value="${basket.carriage}"
								pattern="${CURRENCY_PATTERN}" />
						</c:if> 赠送积分：${basket.pointsSubtotal}
					</td>
					<!-- 操作 -->
					<td><a
				...
				<td height="25" colspan="8">
					<div align="right" style="margin-right: 86px;">
						<b>总计：</b> <span id="totalCurrencyLabel"> <fmt:formatNumber
								...
								pattern="${CURRENCY_PATTERN}" />
						</span>&nbsp;&nbsp; <b>赠送总积分：</b><span>${pointsSubtotalAll}</span> <input
							type="hidden" name="pointsAll" value="${pointsSubtotalAll}" />
					</div>
				</td>
			</tr>
		</tbody>
