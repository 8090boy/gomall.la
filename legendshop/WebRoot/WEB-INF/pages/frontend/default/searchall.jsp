...
<center>
	<table width="98%">
		<tr>
			<td width="140px"
				style="BORDER-RIGHT: #999999 1px solid; vertical-align: top;"
				align="left"><c:if test="${searchArgs.entityType == 0}">
					...
					<br>
						<div>
							...
							<input type="text" id="priceStart" name="priceStart"
								value="${searchArgs.priceStart}" maxlength="8" size="8" /> ￥
						</div>
						<br>
							<div>
								<fmt:message key="to.hint" />
								<input type="text" id="priceEnd" name="priceEnd" maxlength="8"
									value="${searchArgs.priceEnd}" size="8"
				...
				</div> <select class="combox" id="provinceid" name="provinceid"
			...
			<br /> <select class="combox" id="cityid" name="cityid"
			...
			<br /> <select class="combox" id="areaid" name="areaid"
						...
						<div style="margin-left: 20px; font-size: 12pt" align="left">
							...
							<c:forEach items="${requestScope.searchResult}" var="record"
								...
								<table cellpadding="2">
									<tr>
										<td><img
											src="<ls:images item='${record.pic}' scale='2'/>"
											width="100px" height="100px" style="margin-right: 3px" /></td>
												...
												<div>
													<c:if test="${record.price != null}">
														...
														<s><fmt:formatNumber type="currency"
																value="${record.price}" pattern="${CURRENCY_PATTERN}" /></s>
													</c:if>
													<c:if test="${record.cash != null}">
														<fmt:message key="prod_cash" />
														<span style="font-weight: bold;"><fmt:formatNumber
																type="currency" value="${record.cash}"
																pattern="${CURRENCY_PATTERN}" /></span>
													</c:if>
												</div>
												<div>
								 ...
								 <c:if test="${record.city != null}">&raquo; ${record.city}
								 	<c:if test="${record.area != null}">&raquo; ${record.area}</c:if>
												...
												<c:if test="${record.postAddr != null}">
													<div>
														<fmt:message key="detail.address" />
														- ${record.postAddr}
													</div>
												</c:if>
			...
			<td width="255px" valign="top" align="left"><div
					id="visitedshop" name="visitedshop"></div></td>
		</tr>
	</table>
</center>