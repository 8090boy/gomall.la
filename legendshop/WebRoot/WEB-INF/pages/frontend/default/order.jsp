...
<c:if test="${sessionScope.SPRING_SECURITY_LAST_USERNAME !=null}">
	...
	<table class="tables" id="orderTab">
		<form action="${pageContext.request.contextPath}/p/order"
			id="orderForm" name="orderForm" method="post">
			<tr>
				<td class="titlebgnormal" align="left" valign="middle">&nbsp; <input
					type="hidden" id="curPageNO" name="curPageNO" value="${curPageNO}" />
					<input type="hidden" name="subCheck" id="subCheck" value="N" /> <a
					href="javascript:void(0)" onclick="changetab(1);"
					id="processingbutton" name="processingbutton"> <fmt:message
							key="order.processing" />
				</a> <a href="javascript:void(0)" onclick="changetab(2);"
					id="processedbutton" name="processedbutton"><fmt:message
							key="order.processed" /></a> &nbsp;<fmt:message key="order.number" />：<input
					type="text" value="${subForm.subNumber}" maxlength="30"
					name="subNumber" id="subNumber" class="input2" size="30" /> &nbsp;
					<fmt:message key="Order.Status" />： <select id="status"
					name="status" style="height: 1.50em;">
						<ls:optionGroup type="select" required="false" cache="true"
							beanName="ORDER_STATUS" selectedValue="${subForm.status}" />
				</select> <input type="submit" value="<fmt:message key="search"/>" class="s" />
				</td>
			</tr>
		</form>
		<tr>
			<td valign="top"><c:choose>
					<c:when
						test="${requestScope.map != null && fn:length(requestScope.map) > 0}">
						<c:forEach items="${requestScope.map}" var="subList">
							<table class="tables" style="margin: 5px" width="99%" border='0'>
									...
									<td>
										<div
											...
											：<span style="color: red; font-weight: bold;"><a
												href="<ls:domain shopName='${subList.key}' />">${subList.key}</a></span>
											&nbsp;
										</div>
									</td>
								</tr>
								...
								<tr>
									<td>
										<!-- 订单内容 -->
										<table width="100%" cellpadding="0" cellspacing="0" id="col1"
											...
											<c:forEach items="${subList.value}" var="sub">
												<tr>
													<td><div style="padding: 5px;">
															<a
																href='${pageContext.request.contextPath}/p/orderDetail/${sub.subNumber}'
																target="_blank">${sub.subNumber}</a>
														</div></td>
													<td><c:forEach items="${sub.basketList}" var="basket">
															<a target="_blank"
																href="${pageContext.request.contextPath}/views/${basket.prodId}">
																<img src="<ls:images item='${basket.pic}' scale='3'/>"
																width="65px" height="65px"
																style="border-collapse: collapse; margin: 1px; border: 1px solid #CCCCCC;"
																title="${basket.prodName}" />
															</a>
														</c:forEach></td>
													<td>${sub.orderName}</td>
													<td style="color: #FF4400; font-weight: 700"><fmt:formatNumber
															type="currency" value="${sub.subTotal}"
															pattern="${CURRENCY_PATTERN}" /></td>
													<td><fmt:formatDate value="${sub.subDate}"
															pattern="yyyy-MM-dd HH:mm" /></td>
													<td><ls:optionGroup type="label" required="true"
															cache="true" beanName="ORDER_STATUS"
															selectedValue="${sub.status}" /></td>
													<td><c:if test="${sub.status == 1 or sub.status == 7}">
															<a
																href='${pageContext.request.contextPath}/p/orderDetail/${sub.subNumber}'
																target="_blank"><fmt:message key="payment.hint" /></a>
														</c:if> <c:if test="${sub.status == 3}">
															<!-- 3:卖家已经发货 -->
															<!-- 4:交易成功 -->
															<a href="javascript:void(0)"
																onclick='javascript:updateSubStatus("${sub.subId}",4,"${sub.subNumber}");'><fmt:message
																	key="ensure.recieve" /></a>
															<!-- 6:退款中的订单 -->
															<a href="javascript:void(0)"
																onclick='javascript:updateSubStatus("${sub.subId}",6,"${sub.subNumber}");'><fmt:message
																	key="apply.for.refundment" /></a>
														</c:if> <c:if test="${sub.status == 1 or sub.status == 7}">
															<a
																href='javascript:deleteSub("${sub.subId}","${sub.subNumber}");'><fmt:message
																	key="delete" /></a>
														</c:if></td>
												</tr>
											</c:forEach>
					...
					<c:otherwise>
						...
						<tr style="background-color: white; text-align: left;">
							<td><div style="margin-left: 200px;">
									<img
										src="${pageContext.request.contextPath}/common/default/images/basket.png" /><b>您的订单还是空的，赶紧行动吧！</b>
								</div></td>
						</tr>
					</c:otherwise>
				</c:choose></td>
		...
		<tr>
			<td align="right" style="padding: 5px;"><c:out
					value="${toolBar}" escapeXml="${toolBar}"></c:out></td>
		</tr>
	</table>
