...
<script type="text/javascript">
	...
	function sendData(orderBy, orderDir, curPageNO){
	    var data = {
    			...
    			$.ajax({
				url:"${pageContext.request.contextPath}/prodnsortlist/" + $("#sortId").val() +"-" + $("#nsortId").val() + "-" + $("#subnsortId").val() , 
				data: data,
				type:'post', 
...
<div class="right_con">
	...
	<!--产品列表 start -->
	<!--产品列表end -->
	<div id="prodnsortlist" name="prodnsortlist"></div>

</div>
<!-----------right_con end-------------->
<input type="hidden" id="curPageNO" name="curPageNO" value="1" />
<input type="hidden" id="sortId" name="sortId" value="${sortId}" />
<input type="hidden" id="nsortId" name="nsortId" value="${nsortId}" />
<input type="hidden" id="subnsortId" name="subnsortId"
	value="${subnsortId}" />
