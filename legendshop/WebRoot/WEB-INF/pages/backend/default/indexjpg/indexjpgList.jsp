...
%>
		...
		<thead>
			<tr>
				<th><a href="<ls:url address='/admin/index'/>" target="_parent">首页</a>
					&raquo; 商城管理 &raquo; <a
					href="${pageContext.request.contextPath}/admin/indexjpg/query">首页Flash图片管理</a></th>
			</tr>
		</thead>
		<tbody>
			<tr>
				<td>
					<div align="left" style="padding: 3px">
						<form id="form1"
							action="${pageContext.request.contextPath}/admin/indexjpg/query">
							<input type="hidden" id="curPageNO" name="curPageNO"
								value="${curPageNO}">
							<auth:auth ifAnyGranted="F_VIEW_ALL_DATA">
			商城
			<input type="text" name="userName" id="userName" maxlength="50"
									value="${indexJpg.userName}" />
								<input type="submit" value="搜索" />
							</auth:auth>
							<auth:auth ifAnyGranted="FA_SHOP">
								<input type="button" value="创建首页图片"
									onclick='window.location="${pageContext.request.contextPath}/admin/indexjpg/load"' />
							</auth:auth>
						</form>
					</div>
				</td>
	...
	<div align="center">
		...
		<display:table name="list" requestURI="/admin/indexjpg/query"
			...
			<display:column title="图片" style="width:35px">
				<a href="<ls:photo item='${item.img}'/>" target="_blank"><img
					src="<ls:photo item='${item.img}'/>" height="145" width="265" /></a>
			</display:column>
			<display:column title="次序" property="seq" sortable="true"
				sortName="seq" style="width:35px"></display:column>
			<auth:auth ifAnyGranted="FA_SHOP">
				<display:column title="操作" media="html" style="width:80px">
					<c:choose>
						<c:when test="${item.status == 1}">
							<img name="statusImg" itemId="${item.id}"
								itemName="${item.title}" status="${item.status}"
					...
					<a
						href="${pageContext.request.contextPath}/admin/indexjpg/update/${item.id}"
						title="修改"><img alt="修改"
	...
	<table style="width: 100%; border: 0px">
		<tr>
			<td align="left">说明：<br> 1. 首页中上部广告图片，文字均可以加上超链接<br>
				2. 创建首页中上部广告图片，注意:最多上传<b><%=PropertiesUtil.getObject(SysParameterEnum.MAX_INDEX_JPG,Integer.class)%></b>张图片<br>
				3. <img
				src="${pageContext.request.contextPath}/common/default/images/grid_refresh.png"
				title="文字连接">指向图片、文字连接地址<br> 4. <img alt="修改"
				src="<ls:templateResource item='/common/default/images/grid_edit.png'/> ">
				修改按钮<br> 5. <img alt="删除"
