...
<body>
    ...
    %>
	<form
		action="${pageContext.request.contextPath}/admin/shopDetail/query"
			...
			<thead>
				<tr>
					<th><a href="<ls:url address='/admin/index'/>"
						...
						href="${pageContext.request.contextPath}/admin/shopDetail/query">商城管理</a></th>
				</tr>
			</thead>
						...
						<td>
							<div align="left" style="padding: 3px">
								<input type="hidden" id="curPageNO" name="curPageNO"
									value="${curPageNO}" /> 商城&nbsp;<input type="text"
									name="userName" maxlength="50" value="${shopDetail.userName}"
									size="30" /> &nbsp;类型&nbsp; <select id="type" name="type">
								...
								<c:if test="${fn:length(list) == 0}">
									<input type="button" value="创建商城"
										onclick='window.location="${pageContext.request.contextPath}/admin/shopDetail/load"' />
								</c:if>
							</div>
						</td>
					</tr>
	...
	<div align="center">
		...
		<display:table name="list" requestURI="/admin/shopDetail/query"
			...
			<c:if test="${'C2C' == applicationScope.BUSINESS_MODE }">
				<display:column title="商城">
					<a href="<ls:domain shopName='${item.userName}'  />"
						target="_blank">${item.userName}</a>
				</display:column>
			</c:if>
			<display:column title="网站名称" property="siteName">
			</display:column>
			<display:column title="商品数量" property="productNum" sortable="true"
				sortName="productNum" style="width:70px"></display:column>
			<display:column title="下线商品" property="offProductNum" sortable="true"
				sortName="offProductNum" style="width:70px"></display:column>
			<display:column title="状态" class="width:90px">
				<font color="red"> <ls:optionGroup type="label"
						required="false" cache="true" beanName="SHOP_STATUS"
						selectedValue="${item.status}" defaultDisp="" /></font>
				<c:if test="${item.status == -1}">
					<auth:auth ifAnyGranted="FA_SHOP,F_VIEW_ALL_DATA">
			              [<span class="shopStatusSetting cursor_pointer"
							onclick="javascript:auditShop('${item.userId}','${item.shopId}')">审核</span>]
			              </auth:auth>
				</c:if>
			</display:column>
			<auth:auth ifAnyGranted="FA_SHOP,F_VIEW_ALL_DATA">
				<display:column title="操作" media="html" class="actionwidth">
					<a
						href="${pageContext.request.contextPath}/admin/shopDetail/load/${item.shopId}"
						title="修改"><img alt="修改"
						src="<ls:templateResource item='/common/default/images/grid_edit.png'/> "></a>
					<a href='javascript:deleteById("${item.shopId}")' title="删除"><img
						alt="删除"
	...
	<table style="width: 100%; border: 0px">
		<tr>
			<td align="left">说明：<br> 1. 商城为上线状态才可以正常访问<br> 2.
