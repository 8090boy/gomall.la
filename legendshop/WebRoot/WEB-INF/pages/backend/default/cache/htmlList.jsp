...
<body>
		...
		<thead>
			<tr>
				<th><a href="<ls:url address='/admin/index'/>" target="_parent">首页</a>
					&raquo; 系统管理 &raquo; <a
					href="${pageContext.request.contextPath}/admin/system/cache/query">缓存列表</a></th>
			</tr>
		</thead>
		<tbody>
			<tr>
					...
					<div align="left" style="padding: 3px">
						<auth:auth ifAnyGranted="FA_SHOP">
							<input type="button" value="一键清除缓存"
								onclick="javascript:clearHtmlCache('${requestPath}')" />
							<c:if test="${!rootPath}">
								<a href="javascript:enterUpFolder()"><b>上一层</b></a>
							</c:if> &nbsp;  &nbsp;当前路径： ${requestPath} 
</auth:auth>
				</td>
			</tr>
		</tbody>
	...
	<div align="center">
		...
		<form id="queryhtml"
			action="${pageContext.request.contextPath}/admin/cache/queryhtml">
			<input type="hidden" name="requestFolder" id="requestFolder"
				value="${requestFolder }" /> <input type="hidden" name="fileName"
				id="fileName" /> <input type="hidden" name="direct" id="direct" />
			<display:table name="fileList" requestURI="/admin/cache/queryhtml"
				...
				<display:column title="操作">
					<c:choose>
						<c:when test="${item.isFile }">
							...
							<a
								href="javascript:clearHtmlCache('${requestPath}/${item.fileName }')">删除</a>
						</c:when>
						<c:otherwise>
