...
<body>
	<form
		action="${pageContext.request.contextPath}/admin/system/lucene/reindex"
		...
		<div align="center">
				...
				<thead>
					<tr>
						<th><a href="<ls:url address='/admin/index'/>"
							target="_parent">首页</a> &raquo; 系统管理 &raquo; <a
							href="${pageContext.request.contextPath}/admin/system/lucene/query">重建索引</a></th>
					</tr>
				</thead>
			</table>
			...
			<table align="center" class="${tableclass}" id="col1">
					...
					<td>
						<div align="right">
							<input type="radio" id="reindexByDate" value="1" name="type"
								checked="checked">依日期 <font color="ff0000">*</font>
						</div>
					</td>
					<td>开始 <span class="gensmall">(dd/mm/yyyy hh:mm)</span> <script
							type="text/javascript">
									writeOptions("fromDay", 1, 31);
									writeOptions("fromMonth", 1, 12);
									writeOptions("fromYear", 1995, new Date().getFullYear());
									writeTimeGroup("from");
								</script> <br>结束 <span class="gensmall">(dd/mm/yyyy hh:mm)</span> <script
							type="text/javascript">
				...
				<tr>
					...
					<td><input type="radio" id="prodType" name="entityType"
						value="0" checked="checked" /> 商品 <input type="radio"
						id="shopType" name="entityType" / value="1"> 商城 <!-- 
				...
				<tr>
					<td>
						<div align="right">
							依实体Id <font color="ff0000">*</font>
						</div>
					</td>
					<td>开始 <input type="text" name="firstPostId"
						onblur="chooseByPostId(this);" /> 结束 <input type="text"
						name="lastPostId" onBlur="chooseByPostId(this);" />
				...
				<tr>
					...
					<td><input type="checkbox" id="avoidDuplicated"
						name="avoidDuplicatedRecords" value="yes" checked="checked" />
						加入索引前检查文章是否存在,这样可以避免数据重复, 但是建立索引的过程将花较久的时间完成<br> <input
						type="radio" name="indexOperationType" id="operationTypeAppend"
						value="append" checked="checked" /> 加入文件到索引,将纪录加到现存的索引数据库而不重新建立.<br>
						<input type="radio" name="indexOperationType"
						id="operationTypeRecreate" value="recreate" />
						从无到有重建索引,建立一个全新的索引数据库. 这样将会删除所有存在的纪录.</td>
				</tr>
