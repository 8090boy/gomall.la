	...
	<thead>
		<tr>
			<th><a href="<ls:url address='/admin/index'/>" target="_parent">首页</a>
				&raquo; <a
				href="${pageContext.request.contextPath}/admin/order/processing">订单管理</a>
				&raquo; 未处理订单</th>
		</tr>
	</thead>
	<tbody>
		<tr>
			<td>
				<form
					...
					id="processingOrderForm" name="processingOrderForm" method="post"
					style="margin: 0px">
					<input type="hidden" id="curPageNO" name="curPageNO"
						value="${curPageNO}" />
					<div align="left" style="padding: 3px">
						&nbsp; 订单号&nbsp;<input type="text" name="subNumber" id="subNumber"
							value="${subForm.subNumber}" size="30" maxlength="30" /> &nbsp;
						买家&nbsp;<input type="text" name="userName" id="userName"
				...
				<display:table name="list"
					...
					<auth:auth ifAnyGranted="F_VIEW_ALL_DATA">
						<display:column title="商城" property="shopName" sortable="true"
							sortName="shopName" style="width: 100px">
							<a href="<ls:domain shopName='${item.shopName}'  />"
					...
					<display:column title="实收价格" sortable="true" sortName="actualTotal">
						<b><fmt:formatNumber type="currency"
								value="${item.actualTotal}" pattern="${CURRENCY_PATTERN}" /></b>
					</display:column>
					<display:column title="商品" property="prodName" sortable="true"
						sortName="prodName"></display:column>
					<display:column title="买家帐号" sortable="true" sortName="userName"
						style="width: 100px">${item.userName}</display:column>
					...
					<auth:auth ifAnyGranted="FA_SALE">
						<display:column title="操作" media="html" style="width:110px">
							<a href="javascript:void(0)"
								onclick='javascript:orderList.orderDetail("${item.subNumber}");'>详情</a>
							<c:if test="${item.status == 1}">
								<!-- 1: 等待买家付款 -->
								<a href="javascript:void(0)"
									onclick='javascript:orderList2.modifyPrice("${item.total}", "${item.subId}", "${item.subNumber}");'>修改</a>
							</c:if>
							...
							<c:if
								test="${item.payTypeId == 'PGA' &&( item.status != 3 && item.status != 6)}">
								<!-- 3:货到付款,没有退货又没有发货的 -->
								<a href="javascript:void(0)"
									onclick='javascript:updateSubStatus("${item.subId}",3,"${item.subNumber}");'>发货</a>
							</c:if>
							<c:if test="${item.status == 6}">
								...
								<!-- 5:交易关闭 -->
								<a href="javascript:void(0)"
									onclick='javascript:updateSubStatus("${item.subId}",5,"${item.subNumber}");'>同意退货</a>
							</c:if>
...
说明：
<br>
1. 用户支付之后需要点击发货按钮，用户确认收货，整个订单完成，进入历史订单状态
<br>
2. 用户申请退款，在退款之后点击关闭交易，订单完成。
<br>
