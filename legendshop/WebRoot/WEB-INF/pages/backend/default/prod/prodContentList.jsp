...
<display:table name="list" requestURI="/admin/product/queryprodcontent"
	id="item" export="true" class="${tableclass}" style="width:100%;"
	sort="external">
	<display:column
		style="width:55px;vertical-align: middle;text-align: center;"
		title="顺序<input type='checkbox'  id='checkbox' name='checkbox' onClick='javascript:selAll()' />">
		<%=offset++%><input type="checkbox" name="strArray"
			value="${item.prodId}" arg="${item.name}" />
	</display:column>
	...
	<display:column title="商品" style="width:100px">
		<a href="<ls:url address='/views/${item.prodId}'/>" target="_blank"
			title="${item.name}" style="margin: 10px;"> <img width="65"
			height="65" src="<ls:images item='${item.pic}' scale='3'/>"
			alt="${item.name}">
		</a>
	</display:column>
	...
	<auth:auth ifAnyGranted="FA_PROD">
		<display:column title="操作" media="html" style="width: 100px;">
			...
			<a
				href="${pageContext.request.contextPath}/admin/product/update/${item.prodId}"
				title="修改"><img alt="修改"
