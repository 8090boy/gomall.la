...
<c:choose>
	...
	<c:otherwise>
		<a
			href="${pageContext.request.contextPath}/admin/product/update/${prod.prodId}">
			<c:choose>
		...
		<a
			href="${pageContext.request.contextPath}/admin/imgFile/query?productId=${prod.prodId}">
			<c:choose>
		...
		<a
			href="${pageContext.request.contextPath}/admin/dynamic/loadAttribute/${prod.prodId}">
			<c:choose>
				<c:when test="${param.step == 3}">
					<c:choose>
						<c:when test="${DYNAMIC_TYPE == 1}">
							<b>3.动态属性</b>
						</c:when>
		...
		<a
			href="${pageContext.request.contextPath}/admin/dynamic/loadParameter/${prod.prodId}">
			<c:choose>
				<c:when test="${param.step == 3}">
					<c:choose>
						<c:when test="${DYNAMIC_TYPE == 2}">
							<b>4.动态参数</b>
						</c:when>
		...
		<a id="relProdSetp"
			href="javascript:createprodStep.relProduct(${prod.prodId})"> <c:choose>
