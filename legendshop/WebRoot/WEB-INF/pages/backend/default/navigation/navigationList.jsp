...
<body>
	...
	%>
	<form action="<ls:url address='/admin/system/navigation/query'/>"
			...
			<thead>
				<tr>
					<th><a href="<ls:url address='/admin/index'/>"
						...
						href="<ls:url address='/admin/system/navigation/query'/>">网站导航</a>
					</th>
				</tr>
			</thead>
			<tbody>
				<tr>
					<td>
						<div align="left" style="padding: 3px">
							<input type="button" value="创建网站导航"
								onclick='window.location="<ls:url address='/admin/system/navigation/load'/>"' />
						</div>
					</td>
				</tr>
			</tbody>
	...
	<div align="center">
		...
		<display:table name="list" requestURI="/admin/system/navigation/query"
			id="parent" export="false" sort="external" class="${tableclass}"
			style="width:100%">
			...
			<display:column title="二级导航">
				<c:if test="${fn:length(subItemList) > 0}">
					<display:table name="subItemList" id="child" cellpadding="0"
						cellspacing="0">
						<display:column title="名称" property="name"></display:column>
						...
						<display:column title="操作" media="html" autolink="true"
							...
							<c:choose>
								<c:when test="${child.status == 1}">
									<img name="childStatusImg" itemId="${child.itemId}"
							...
							<a
								href="<ls:url address='/admin/system/navigationItem/load/${child.itemId}'/>"
								title="修改"> <img alt="修改"
								src="<ls:templateResource item='/common/default/images/grid_edit.png'/>">
							</a>
							<a href='javascript:deleteBychildId("${child.itemId}")'><img
								alt="删除"
			...
			<display:column title="操作" media="html" style="width:100px">
				<c:choose>
					<c:when test="${parent.status == 1}">
						<img name="parentStatusImg" itemId="${parent.naviId}"
							itemName="${parent.name}" status="${parent.status}"
							class="cursor_pointer"
							src="<ls:templateResource item='/common/default/images/blue_down.png'/> ">
					</c:when>
					<c:otherwise>
				...
				<a
					href="<ls:url address='/admin/system/navigation/querysearth/${parent.naviId}'/>"
					title="创建二级导航"> <img
					src="<ls:templateResource item='/common/default/images/grid_add.png'/>">
				</a>
				<a
					href="<ls:url address='/admin/system/navigation/load/${parent.naviId}'/>"
					title="修改"> <img alt="修改"
					src="<ls:templateResource item='/common/default/images/grid_edit.png'/>">
				</a>
