...
<body>
	...
	<form
		action="${pageContext.request.contextPath}/admin/externallink/query"
		...
		<table class="${tableclass}" style="width: 100%">
			<thead>
				<tr>
					<th><a href="<ls:url address='/admin/index'/>"
						...
						href="${pageContext.request.contextPath}/admin/externallink/query">链接管理</a></th>
				</tr>
			</thead>
			<tbody>
				<tr>
          ...
          商城&nbsp; <input type="text" name="userName" maxlength="50"
							...
							<auth:auth ifAnyGranted="FA_SHOP">
								<input type="button" value="创建友情链接"
									onclick='window.location="${pageContext.request.contextPath}/admin/externallink/load"' />
							</auth:auth>
						</div>
					</td>
				</tr>
	...
	<div align="center">
		...
		<display:table name="list" requestURI="/admin/externallink/query"
			...
			<display:column title="图片">
				<c:if test="${item.picture != null}">
					<img src="<ls:photo item='${item.picture}'/>" height="31px"
						width="81px" />
				</c:if>
			</display:column>
			...
			<display:column title="次序" property="bs" sortable="true"
				sortName="bs"></display:column>
			<auth:auth ifAnyGranted="FA_SHOP">
				<display:column title="操作" media="html" class="actionwidth">
					<a
						href="${pageContext.request.contextPath}/admin/externallink/load/${item.id}"
						title="修改"><img alt="修改"
	...
	<table style="width: 100%; border: 0px">
		<tr>
			<td align="left">说明：<br> 1. 友情链接管理，将会出现在商城首页<br> 2.
				可以为每个友情链接增加一个图片<br>
			</td>
		<tr>
	</table>
