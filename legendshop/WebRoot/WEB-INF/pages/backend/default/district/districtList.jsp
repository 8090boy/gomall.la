...
<body>
	<form action="<ls:url address='/admin/system/district/query'/>"
			...
			<thead>
				<tr>
					<th><a href="<ls:url address='/admin/index'/>"
						...
						href="<ls:url address='/admin/system/district/query'/>">地区管理</a></th>
				</tr>
			</thead>
		</table>
	</form>
	您可以自己添加地区数据。
	</br> 添加，编辑或删除操作后需要点击“提交”按钮才生效。
	<div align="left">
		选择地区： <select class="combox" id="provinceid" name="provinceid"
			requiredTitle="true" childNode="cityid"
			selectedValue="${entity.provinceid}"
			retUrl="${pageContext.request.contextPath}/common/loadProvinces"
			onchange="changeCities(this.value)">
		</select> <select class="combox" id="cityid" name="cityid" requiredTitle="true"
			selectedValue="${entity.cityid}" showNone="false"
			parentValue="${entity.provinceid}" childNode="areaid"
			retUrl="${pageContext.request.contextPath}/common/loadCities/{value}"
			onchange="changAreas(this.value)">
		</select> <select class="combox" id="areaid" name="areaid" requiredTitle="true"
			selectedValue="${entity.areaid}" showNone="false"
			parentValue="${entity.cityid}"
	...
	<input type="button" value="增加" style="float: left"
		onclick="javascript:addDisctrct()" />
