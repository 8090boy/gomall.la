...
<body>
	<form action="<ls:url address='/admin/productProperty/query'/>"
		id="form1" method="post">
		<table class="${tableclass}" style="width: 100%">
			<thead>
				<tr>
					<th><a href="<ls:url address='/admin/index'/>"
						target="_parent">首页</a> &raquo; 商品管理 &raquo; 规格管理 &raquo; <a
						href="<ls:url address='/admin/productProperty/query'/>">产品规格</a></th>
				</tr>
			</thead>
		</table>
		<input type="hidden" id="curPageNO" name="curPageNO"
			value="${curPageNO}" /> 规格名称&nbsp;<input type="text" name="propName"
			maxlength="50" value="${productProperty.propName}" /> <input
			type="submit" value="搜索" /> <input type="button" value="创建规格"
			onclick='window.location="<ls:url address='/admin/productProperty/load'/>"' />
		<br>
	</form>
	<div align="center">
		<%@ include file="/WEB-INF/pages/common/messages.jsp"%>
		<display:table name="list" requestURI="/admin/productProperty/query"
				...
				<display:column title="操作" media="html" style="width: 80px;">
					<c:choose>
						<c:when test="${item.status == 1}">
							<img name="statusImg" item_id="${item.propId}"
								itemName="${item.propName}" status="${item.status}"
					...
					<a
						href="${pageContext.request.contextPath}/admin/productProperty/load/${item.propId}"
						title="修改"><img alt="修改"
						src="<ls:templateResource item='/common/default/images/grid_edit.png'/> "></a>
					<a
						href='javascript:confirmDelete("${item.propId}","${item.propName}")'
						title="删除"><img alt="删除"
