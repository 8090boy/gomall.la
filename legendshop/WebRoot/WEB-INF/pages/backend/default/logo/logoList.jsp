...
<body>
	...
	%>
	<form action="${pageContext.request.contextPath}/admin/logo/query"
		...
		<table class="${tableclass}" style="width: 100%">
			<thead>
				<tr>
					<th><a href="<ls:url address='/admin/index'/>"
						...
						href="${pageContext.request.contextPath}/admin/logo/query">Logo管理</a></th>
				</tr>
			</thead>
			<tbody>
				<tr>
			...
			<input type="text" name="userName" maxlength="50"
							...
							<c:if test="${fn:length(list) == 0}">
								<input type="button" value="创建首页Logo"
									onclick='window.location="${pageContext.request.contextPath}/admin/logo/load"' />
							</c:if>
						</div>
					</td>
				</tr>
	...
	<div align="center">
		...
		<display:table name="list" requestURI="/admin/logo/query" id="item"
			...
			<display:column title="图片">
				<img src="<ls:photo item='${item.logoPic}'/>" height="65px"
					width="180px" />
			</display:column>
			<auth:auth ifAnyGranted="FA_SHOP">
				<display:column title="操作" media="html" class="actionwidth">
					<a
						href="${pageContext.request.contextPath}/admin/logo/load/${item.userId}"
						title="修改"><img alt="修改"
						src="<ls:templateResource item='/common/default/images/grid_edit.png'/> "></a>
					<a href='javascript:deleteById("${item.userId}")' title="删除"><img
						alt="删除"
	...
	<table style="width: 100%; border: 0px">
		<tr>
			<td align="left">说明：<br> 1. 每个商城可以设置自己的Logo，如果不设置则采用默认Logo<br>
			</td>
		<tr>
	</table>
