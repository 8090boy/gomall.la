...
<body>
	...
	<table class="${tableclass}" style="width: 100%">
		...
		<tbody>
			<tr>
				<td>
					<form
						action="<ls:url address='/admin/prodspec/query/${dynamicTemp.type}'/>"
            ...
            <input type="text" name="userName" maxlength="50"
									value="${dynamicTemp.userName}" size="15" />
							</auth:auth>
							<auth:auth ifNotGranted="F_VIEW_ALL_DATA">
			  商品类型&nbsp;
		    <lb:sortOption id="sortId" type="P"
									selectedValue="${dynamicTemp.sortId}" />
							</auth:auth>
						</div>
						<div align="left" style="padding: 3px">
							商品规格名称&nbsp; <input type="text" name="name" id="name"
								maxlength="50" value="${dynamicTemp.name}" size="32" /> 状态 <select
								...
								value="创建商品规格"
								onclick="window.location='${pageContext.request.contextPath}/admin//prodspec/createattribute/${dynamicTemp.type}'" />
						</div>
					</form>
				</td>
			</tr>
	...
	<div align="center">
		...
		<display:table name="list" requestURI="/admin/prodspec/query"
			...
			<display:column title="操作" style="width: 80px">
				<auth:auth ifAnyGranted="FA_PROD">
					...
					<a
						href="${pageContext.request.contextPath}/admin/prodspec/attribute/${item.id}"
						title="修改"><img alt="修改"
						src="<ls:templateResource item='/common/default/images/grid_edit.png'/> "></a>
					<a href='javascript:deleteById("${item.id}")'><img alt="删除"
