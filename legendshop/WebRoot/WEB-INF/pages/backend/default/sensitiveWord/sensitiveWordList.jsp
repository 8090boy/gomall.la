...
<%
	...
	%>
<body>
	<form action="<ls:url address='/admin/system/sensitiveWord/query'/>"
		id="form1" method="post">
		<table class="${tableclass}" style="width: 100%">
			<thead>
				<tr>
					<th><a href="<ls:url address='/admin/system/index'/>"
						target="_parent">首页</a> &raquo; 商城管理 &raquo; <a
						href="<ls:url address='/admin/system/sensitiveWord/query'/>">敏感字过滤表</a>
					</th>
				</tr>
			</thead>
			<tbody>
					...
					<td>
						<div align="left" style="padding: 3px">
							<input type="hidden" id="curPageNO" name="curPageNO"
								value="${curPageNO}" /> &nbsp; 关键字： <input type="text"
								name="words" id="words" maxlength="50"
								value="${sensitiveWord.words}" /> <input type="submit"
								value="搜索" /> <input type="button" value="创建敏感字过滤表"
								onclick='window.location="<ls:url address='/admin/system/sensitiveWord/load'/>"' />
						</div>
					</td>
				</tr>
			</tbody>
	...
	<div align="center">
		...
		<display:table name="list"
			requestURI="/admin/system/sensitiveWord/query" id="item"
			...
			<display:column title="是否全局敏感字" style="width:100px">
				<ls:optionGroup type="label" required="true" cache="true"
					beanName="YES_NO" selectedValue="${item.isGlobal}" />
			</display:column>
			<display:column title="操作" media="html" style="width: 80px;">
				<a
					href="<ls:url address='/admin/system/sensitiveWord/load/${item.sensId}'/>"
					title="修改"> <img alt="修改"
					src="<ls:templateResource item='/common/default/images/grid_edit.png'/>">
				</a>
				<a href='javascript:deleteById("${item.sensId}")' title="删除"> <img
					alt="删除"
