...
<body>
    ...
    %>
	<form action="${pageContext.request.contextPath}/admin/nsort/query"
			...
			<thead>
				<tr>
					<th><a href="<ls:url address='/admin/index'/>"
						...
						href="${pageContext.request.contextPath}/admin/sort/query">类型管理</a>
						&raquo; <a
						href="${pageContext.request.contextPath}/admin/nsort/query?sortId=${param.sortId}">二级商品类型管理</a></th>
				</tr>
			</thead>
			<tbody>
				<tr>
					<td>
						<div align="left" style="padding: 3px">
							<input type="hidden" id="curPageNO" name="curPageNO"
								value="${curPageNO}" /> <input type="hidden" id="sortId"
								name="sortId" value="${nsort.sortId}" /> 商品分类： <b>${nsort.sortName}</b>，
							二级类型 <input type="text" name="nsortName" id="nsortName"
								maxlength="50" value="${name}" /> <input type="submit"
								value="搜索" /> <input type="button" value="创建二级商品类型"
								onclick='window.location="${pageContext.request.contextPath}/admin/nsort/load?sortId=${nsort.sortId}"' />
						</div>
					</td>
				</tr>
			</tbody>
							...
							<auth:auth ifAnyGranted="FA_SHOP">
								<c:choose>
									<c:when test="${child.status == 1}">
										<img name="statusImg" itemId="${child.nsortId}"
											itemName="${child.nsortName}" status="${child.status}"
								...
								<a
									href="${pageContext.request.contextPath}/admin/nsort/update/${child.nsortId}"><img
									alt="修改三级商品类型"
									src="<ls:templateResource item='/common/default/images/grid_edit.png'/> "
									title="修改三级商品类型"></a>
								<a href='javascript:deleteById("${child.nsortId}")'><img
									alt="删除"
				...
				<auth:auth ifAnyGranted="FA_SHOP">
					<c:choose>
						<c:when test="${parent.status == 1}">
							<img name="statusImg" itemId="${parent.nsortId}"
								itemName="${parent.nsortName}" status="${parent.status}"
								class="cursor_pointer"
								src="<ls:templateResource item='/common/default/images/blue_down.png'/> ">
						</c:when>
						<c:otherwise>
					...
					<a
						href="${pageContext.request.contextPath}/admin/nsort/load?sortId=${nsort.sortId}&parentNsortId=${parent.nsortId}"><img
						alt="创建三级商品类型"
						src="${pageContext.request.contextPath}/common/default/images/grid_add.png"
						title="创建三级商品类型"></a>
					<a
						href="${pageContext.request.contextPath}/admin/nsort/update/${parent.nsortId}"><img
						alt="修改二级商品类型"
						src="<ls:templateResource item='/common/default/images/grid_edit.png'/> "
						title="修改二级商品类型"></a>
					<a href='javascript:deleteById("${parent.nsortId}")'><img
						alt="删除"
						src="<ls:templateResource item='/common/default/images/grid_delete.png'/> "
						title="删除二级商品类型"></a>
				</auth:auth>
