...
<body>
	<form action="${pageContext.request.contextPath}/admin/nsort/save"
			...
			<thead>
				<tr>
					<th><a href="<ls:url address='/admin/index'/>"
						...
						&raquo; <a
						href="${pageContext.request.contextPath}/admin/nsort/query?sortId=${nsort.sortId}">二级商品类型管理</a>
						&raquo; 创建二级商品类型 <c:if test="${nsort.parentNsortId != null}">  &raquo; 创建三级商品类型</c:if>
					</th>
				</tr>
			</thead>
		</table>
		<input id="nsortId" name="nsortId" value="${nsort.nsortId}"
			type="hidden"> <input id="sortId" name="sortId"
			value="${nsort.sortId}" type="hidden"> <input
			id="parentNsortId" name="parentNsortId"
			value="${nsort.parentNsortId}" type="hidden">
		<div align="center">
			<table align="center" class="${tableclass}" id="col1">
				...
				<tr>
					<td>
						<div align="center">
							商品类型: <font color="ff0000">*</font>
						</div>
					</td>
					<td><input type="text" name="nsortName" id="nsortName"
						value="${nsort.nsortName}" /></td>
				</tr>
				<tr>
					<td>
						<div align="center">排序(必须为数字)：</div>
					</td>
					<td><input type="text" name="seq" id="seq"
						value="${nsort.seq}" /></td>
				</tr>
				<c:if test="${nsort.parentNsortId == null}">
					<tr>
						...
						<td><select id="sortDeputy" name="sortDeputy">
				...
				<c:if test="${nsort.parentNsortId != null && nsort.nsortId != null}">
					<tr>
						...
						<td><a href="javascript:nsort.appendBrand(${nsort.nsortId})">增加品牌</a>&nbsp;还没有品牌？请先&nbsp;<a
