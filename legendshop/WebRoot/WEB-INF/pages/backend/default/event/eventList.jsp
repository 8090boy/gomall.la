...
<body>
	<form action="<ls:url address='/admin/event/query'/>" id="form1"
		method="post">
		<table class="${tableclass}" style="width: 100%">
			<thead>
				<tr>
					<th><a href="<ls:url address='/admin/index'/>"
						target="_parent">首页</a> &raquo; 日志管理 &raquo; <a
						href="<ls:url address='/admin/event/query'/>">系统日志</a></th>
				</tr>
			</thead>
			<tbody>
				<tr>
					<td><input type="hidden" id="curPageNO" name="curPageNO"
						...
						<div align="left" style="padding: 5px;">
							&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;操作人<input type="text"
								name="modifyUser" maxlength="50" value="${event.modifyUser}" />&nbsp;
							用户名称<input type="text" name="userName" maxlength="50"
								value="${event.userName}" />&nbsp; 对象<input type="text"
								name="type" maxlength="50" value="${event.type}" />
						</div>
						<div align="left" style="padding: 5px;">
							&nbsp;开始时间 <input readonly="readonly" name="startTime"
								id="startTime" class="Wdate" type="text" onClick="WdatePicker()"
								value='<fmt:formatDate value="${event.startTime}" pattern="yyyy-MM-dd"/>' />
							&nbsp;结束时间 <input readonly="readonly" name="endTime" id="endTime"
								class="Wdate" type="text" onClick="WdatePicker()"
								value='<fmt:formatDate value="${event.endTime}" pattern="yyyy-MM-dd"/>' />
	...
	<div align="center">
		...
		<display:table name="list" requestURI="/admin/event/query" id="item"
			...
			<display:column title="时间" property="createTime" sortable="true"
				sortName="createTime"></display:column>
			<display:column title="操作" media="html" style="width: 60px">
				<a href="<ls:url address='/admin/event/load/${item.eventId}'/>"
					title="修改"> <img alt="修改"
