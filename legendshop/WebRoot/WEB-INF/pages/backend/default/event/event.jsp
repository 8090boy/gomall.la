...
<body>
	<form action="<ls:url address='/admin/event/save'/>" method="post"
		id="form1">
		<input id="eventId" name="eventId" value="${event.eventId}"
			type="hidden">
		<table align="center" class="${tableclass}" id="col1"
			...
			<tr>
				<td>
					<div>
						操作人：${event.modifyUser}，用户姓名：${event.userName}，对象：${event.type}，类型
						：${event.operation }， 时间：${event.createTime}</div>
					<div>
