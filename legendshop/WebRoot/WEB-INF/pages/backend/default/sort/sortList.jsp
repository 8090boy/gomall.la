		...
		<thead>
			<tr>
				<th><a href="<ls:url address='/admin/index'/>" target="_parent">首页</a>
					&raquo; 商品管理 &raquo; <a
					href="${pageContext.request.contextPath}/admin/sort/query">类型管理</a></th>
			</tr>
		</thead>
		<tbody>
			<tr>
				<td>
					<form id="form1"
						action="${pageContext.request.contextPath}/admin/sort/query"
						method="post">
			...
			<input type="text" name="userName" id="userName" maxlength="50"
						...
						<div align="left" style="padding: 3px">
							...
							<auth:auth ifAnyGranted="FA_PROD">
								<input type="button" value="创建商品类型"
									onclick='window.location="${pageContext.request.contextPath}/admin/sort/load"' />
							</auth:auth>
				...
				<display:column title="操作" media="html" style="width:100px">
					<c:choose>
						<c:when test="${item.status == 1}">
							<img name="statusImg" itemId="${item.sortId}"
					...
					<c:if test="${item.sortType ne 'G'}">
						<a
							href="${pageContext.request.contextPath}/admin/nsort/query?sortId=${item.sortId}"
							title="创建二级商品类型"><img
							src="<ls:templateResource item='/common/default/images/grid_add.png'/>"></a>
					</c:if>
					<a
						href="${pageContext.request.contextPath}/admin/sort/update/${item.sortId}"
						title="修改"><img
