...
<link
	href="<ls:templateResource item="/common/default/css/category-min.css" />"
	rel="stylesheet" type="text/css" media="screen" />
<link href="<ls:templateResource item="/common/default/css/core.css" />"
	rel="stylesheet" type="text/css" media="screen" />
</head>
<body scroll="no">
	<div id="layout">
				...
				<div style="display: block;" class="page">
					...
					<div class="pageContent" style="padding: 5px">
						<div class="cate-main">
									...
									<ol class="cc-list" id="cc-list" style="left: 0px;">
										<li class="cc-list-item" tabindex="-1" style="">
											<div class="cc-cbox-filter">
												<label for="cc-cbox-filter1">输入一级商品分类</label> <input
													id="cc-cbox-filter1" class="lebelinput"
													name="cc-cbox-filter1" style="width: 176px;">
											</div>
											<div class="cc-tree">
													...
													<li class="cc-tree-group  cc-tree-expanded">
														<ul class="cc-tree-gcont" id="sortList" name="sortList">
															<c:forEach items="${requestScope.sortList }" var="sort">
																<li id="${sort.sortId}"
																	class='cc-tree-item <c:if test="${fn:length(sort.nsort)>0}">cc-hasChild-item</c:if>'>${sort.sortName }</li>
															</c:forEach>
														</ul>
													</li>
												</ul>
							...
							<form name="mainform"
								action="<ls:url address='/admin/product/load'/>" method="post"
								id="mainform">
								<input type="hidden" id="sortId" name="sortId" /> <input
									type="hidden" id="nsortId" name="nsortId" /> <input
									type="hidden" id="subsortId" name="subsortId" /> <input
									type="hidden" id="brandId" name="brandId" /> <input
									type="hidden" id="prodId" name="prodId" value="${prodId}" />
								<fieldset>
									<div class="cateBottom">
										<span class="cateBtn catePublish"><button
												id="catePubBtn" type="button">我已阅读以下规则，现在发布</button> </span>
									</div>
								</fieldset>
							</form>
