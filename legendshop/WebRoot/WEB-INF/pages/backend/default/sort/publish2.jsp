...
<div class="cc-cbox-filter">
	<label for="cc-cbox-filter2">输入二级商品分类</label><input
		id="cc-cbox-filter2" class="lebelinput" style="width: 175px;">
</div>
<ul role="listbox" class="cc-cbox-cont" id="nsortList" name="nsortList">
	<c:forEach items="${requestScope.nSort2List }" var="nsort">
		<li id="cc-cbox-item${nsort.nsortId}" sortId="${nsort.nsortId}"
			class='cc-cbox-item  <c:if test="${fn:length(nsort.subSort)>0}">cc-hasChild-item</c:if>'>
			<!-- 
											    <i>A</i>
											     --> ${nsort.nsortName }
		</li>
