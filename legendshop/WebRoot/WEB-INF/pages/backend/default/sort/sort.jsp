...
<body>
	<form action="${pageContext.request.contextPath}/admin/sort/save"
		...
		<table class="${tableclass}" style="width: 100%" id="col1">
			...
			<tbody>
				<tr>
					<td width="200px"><div align="right">
							类型：<font color="#ff0000">*</font>
						</div></td>
					<td><input type="text" name="sortName" id="sortName"
						size="30" value="${sort.sortName}"> <input id="sortId"
						name="sortId" value="${sort.sortId}" type="hidden"></td>
				</tr>
				<tr>
					<td><div align="right">次序(必须为数字)：</div></td>
					<td><input type="text" name="seq" id="seq" size="30"
						value="${sort.seq}"></td>
				</tr>
				<tr>
					<td><div align="right">Header菜单：</div></td>
					<td><select id="headerMenu" name="headerMenu">
				...
				<tr>
					...
					<td><select id="navigationMenu" name="navigationMenu">
							<ls:optionGroup type="select" required="true" cache="true"
				...
				<c:if test="${sort.brandList != null}">
					<tr>
						...
						<td><c:forEach var="brand" items="${sort.brandList}">
						   ${brand.brandName } &nbsp;
					</c:forEach></td>
					</tr>
				</c:if>
				...
				<tr>
					<td colspan="2"><div align="center">
							...
							<input type="reset" value="重置" /> <input type="button" value="返回"
								onclick='window.location="${pageContext.request.contextPath}/admin/sort/query"' />
						</div></td>
				</tr>
			</tbody>
		</table>
