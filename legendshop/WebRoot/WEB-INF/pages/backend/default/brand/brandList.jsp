...
<body>
    ...
    %>
	<form action="${pageContext.request.contextPath}/admin/brand/query"
		...
		<table class="${tableclass}" style="width: 100%">
			<thead>
				<tr>
					<th><a href="<ls:url address='/admin/index'/>"
						...
						href="${pageContext.request.contextPath}/admin/brand/query">品牌管理</a></th>
				</tr>
			</thead>
			<tbody>
				<tr>
            ...
            商城&nbsp;<input type="text" name="userName" maxlength="50"
							...
							<auth:auth ifAnyGranted="FA_PROD">
								<input type="button" value="创建品牌"
									onclick='window.location="${pageContext.request.contextPath}/admin/brand/load"' />
							</auth:auth>
	...
	<div align="center">
		...
		<display:table name="list" requestURI="/admin/brand/query" id="item"
			...
			<display:column title="次序" property="seq" sortable="true"
				sortName="seq" style="width:50px"></display:column>
			<auth:auth ifAnyGranted="FA_PROD">
				<display:column title="操作" media="html" style="width:80px">
					<c:choose>
						<c:when test="${item.status == 1}">
							<img name="statusImg" itemId="${item.brandId}"
								itemName="${item.brandName}" status="${item.status}"
					...
					<a
						href="${pageContext.request.contextPath}/admin/brand/update/${item.brandId}"
						title="修改"><img alt="修改"
						src="<ls:templateResource item='/common/default/images/grid_edit.png'/> "></a>
					<a href='javascript:deleteById("${item.brandId}")' title="删除"><img
						alt="删除"
	...
	<table style="width: 100%; border: 0px">
		<tr>
			<td align="left">说明：<br> 1. 品牌带有一个品牌图片<br> 2.
				品牌可以挂在商品三级类型上，在用户选择了三级类型之后出现对应的品牌<br> 3. <img alt="修改"
				src="<ls:templateResource item='/common/default/images/grid_edit.png'/> ">
				修改按钮<br> 4. <img alt="删除"
				src="<ls:templateResource item='/common/default/images/grid_delete.png'/> ">
				删除按钮，删除品牌，如果删除品牌则不影响其下的商品<br>
			</td>
		<tr>
	</table>
