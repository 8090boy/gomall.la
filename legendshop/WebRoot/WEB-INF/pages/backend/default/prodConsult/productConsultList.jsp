...
<body>
	...
	%>
	<form
		action="${pageContext.request.contextPath}/admin/productConsult/query"
						...
						<div align="left" style="padding: 3px">
							...
							<auth:auth ifAnyGranted="F_VIEW_ALL_DATA">
								 商城&nbsp;<input type="text" name="userName" maxlength="50"
									value="${productConsult.userName}" />
							</auth:auth>
							商品名称&nbsp;<input type="text" name="prodName" maxlength="50"
								value="${productConsult.prodName}" /> 类型&nbsp; <select
								id="pointType" name="pointType"><ls:optionGroup
									type="select" required="false" cache="true"
									beanName="CONSULT_TYPE"
									selectedValue="${productConsult.pointType}" /></select> 已回复&nbsp; <select
								id="replyed" name="replyed">
		...
		<display:table name="list" requestURI="/admin/productConsult/query"
			...
			<display:column title="商品">
				<a href="<ls:url address='/views/${item.prodId}'/>" target="_blank"
					title="${item.prodName}">${item.prodName}</a>
			</display:column>
			...
			<display:column title="操作" media="html" class="actionwidth">
				<a
					href="${pageContext.request.contextPath}/admin/productConsult/load/${item.id}"
					title="回复"><img alt="回复"
	...
	<table style="width: 100%; border: 0px">
		<tr>
			<td align="left">说明：<br> 1. 用户在前台商品页面中输入咨询内容<br>
			</td>
		<tr>
	</table>
