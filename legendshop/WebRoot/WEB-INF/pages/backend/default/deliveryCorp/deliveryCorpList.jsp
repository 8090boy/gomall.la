...
<body>
	<form action="<ls:url address='/admin/deliveryCorp/query'/>" id="form1"
		method="post">
			...
			<thead>
				<tr>
					<th><a href="<ls:url address='/admin/index'/>"
						...
						href="<ls:url address='/admin/deliveryCorp/query'/>">物流公司</a></th>
				</tr>
			</thead>
			<tbody>
				<tr>
					<td>
		            	...
		            	商城&nbsp;<input type="text" name="userName" maxlength="50"
									value="${deliveryCorp.userName}" />
							</auth:auth>
							物流公司名称&nbsp;<input type="text" name="name" maxlength="50"
								value="${deliveryCorp.name}" /> <input type="submit" value="搜索" />
							<auth:auth ifAnyGranted="FA_SHOP">
								<input type="button" value="创建物流公司"
									onclick='window.location="<ls:url address='/admin/deliveryCorp/load'/>"' />
							</auth:auth>
						</div>
					</td>
				</tr>
	...
	<div align="center">
		...
		<display:table name="list"
			requestURI="<ls:url address='/admin/deliveryCorp/query'/>" id="item"
			...
			<auth:auth ifAnyGranted="FA_SHOP">
				<display:column title="操作" media="html">
					<a
						href="<ls:url address='/admin/deliveryCorp/load/${item.dvyId}'/>"
						title="修改"> <img alt="修改"
						src="<ls:templateResource item='/common/default/images/grid_edit.png'/>">
					</a>
					<a href='javascript:deleteById("${item.dvyId}")' title="删除"> <img
						alt="删除"
