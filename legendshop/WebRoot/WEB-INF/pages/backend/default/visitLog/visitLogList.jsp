...
<body>
    ...
    %>
	<form action="${pageContext.request.contextPath}/admin/visitLog/query"
		id="form1" method="post">
		<table class="${tableclass}" style="width: 100%">
			<thead>
				<tr>
					<th><a href="<ls:url address='/admin/index'/>"
						target="_parent">首页</a> &raquo; 报表管理 &raquo; <a
						href="${pageContext.request.contextPath}/admin/visitLog/query">浏览历史</a></th>
				</tr>
			</thead>
			<tbody>
				<tr>
         ...
         &nbsp; 商城 <input type="text" name="shopName" maxlength="50"
									value="${bean.shopName}" />
							</auth:auth>
							&nbsp;开始&nbsp; <input readonly="readonly" name="startTime"
								id="startTime" class="Wdate" type="text" onClick="WdatePicker()"
								value='<fmt:formatDate value="${bean.startTime}" pattern="yyyy-MM-dd"/>' />
							&nbsp;结束 <input readonly="readonly" name="endTime" id="endTime"
								class="Wdate" type="text" onClick="WdatePicker()"
								value='<fmt:formatDate value="${bean.endTime}" pattern="yyyy-MM-dd"/>' />
						</div>
						<div align="left" style="padding: 3px">
							用户名&nbsp;<input type="text" name="userName" maxlength="50"
								value="${bean.userName}" /> &nbsp;页面 <select id="page"
								name="page">
								<ls:optionGroup type="select" required="false" cache="true"
									beanName="VISIT_LOG_TYPE" selectedValue="${bean.page}" />
							</select> &nbsp;商品 <input type="text" name="productName" maxlength="50"
								value="${bean.productName}" /> <input type="submit" value="搜索" />
						</div>
					</td>
				</tr>
			</tbody>
	...
	<div align="center">
		...
		<display:table name="list" requestURI="/admin/visitLog/query"
			...
			<display:column title="IP" property="ip" sortable="true"
				sortName="ip"></display:column>
			<display:column title="国家" property="country" sortable="true"
				sortName="country"></display:column>
			<auth:auth ifAnyGranted="F_VIEW_ALL_DATA">
				<display:column title="地区" property="area" sortable="true"
					sortName="area"></display:column>
				<display:column title="商城名" property="shopName" sortable="true"
					sortName="shopName"></display:column>
			</auth:auth>
			<display:column title="商品名">
				<c:if test="${item.productName != null}">
					<a
						href="${pageContext.request.contextPath}/views/${item.productId}"
	...
	<table style="width: 100%; border: 0px">
		<tr>
			<td align="left">说明：<br> 1. 页面访问历史，记录用户于何时何地访问您的商城<br>
