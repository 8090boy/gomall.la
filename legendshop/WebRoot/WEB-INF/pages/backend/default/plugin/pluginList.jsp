		...
		<thead>
			<tr>
				<th><a href="<ls:url address='/admin/index'/>" target="_parent">首页</a>
					&raquo; 系统管理 &raquo; <a
					href="${pageContext.request.contextPath}/admin/system/plugin/query">插件列表</a></th>
			</tr>
		</thead>
	</table>
			...
			<display:column title="操作" style="width: 80px">
				<c:choose>
					<c:when test="${item.pluginConfig.status == 'Y' }">
						<img name="statusImg" item_id="${item.pluginConfig.pulginId}"
							itemName="${item.pluginConfig.pulginId}"
							status="${item.pluginConfig.status}" class="cursor_pointer"
