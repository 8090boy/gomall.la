...
<body>
	<form action="<ls:url address='/admin/productPropertyValue/query'/>"
			...
			<thead>
				<tr>
					<th><a href="<ls:url address='/admin/index'/>"
						...
						href="<ls:url address='/admin/productPropertyValue/query'/>">属性值</a>
					</th>
				</tr>
			</thead>
		</table>
		...
		<auth:auth ifAnyGranted="F_VIEW_ALL_DATA">
            	商城名称&nbsp;<input type="text" name="userName" maxlength="50"
				value="${productPropertyValue.userName}" />
		</auth:auth>
		<input type="submit" value="搜索" /> <input type="button" value="创建属性值"
			onclick='window.location="<ls:url address='/admin/productPropertyValue/load'/>"' />
		<br>
	</form>
	<div align="center">
		<%@ include file="/WEB-INF/pages/common/messages.jsp"%>
		<display:table name="list"
			requestURI="/admin/productPropertyValue/query" id="item"
			...
			<display:column title="Action" media="html">
				<a
					href="<ls:url address='/admin/productPropertyValue/load/${item.valueId}'/>"
					title="修改"> <img alt="修改"
					src="<ls:templateResource item='/img/grid_edit.png'/>">
				</a>
				<a href='javascript:deleteById("${item.valueId}")' title="删除"> <img
					alt="删除" src="<ls:templateResource item='/img/grid_delete.png'/>">
				</a>
			</display:column>
		</display:table>
