...
<body>
    ...
    %>
	<form
		action="${pageContext.request.contextPath}/admin/newsCategory/query"
			...
			<thead>
				<tr>
					<th><a href="<ls:url address='/admin/index'/>"
						...
						href="${pageContext.request.contextPath}/admin/newsCategory/query">栏目管理</a></th>
				</tr>
			</thead>
			<tbody>
				<tr>
					<td>
						<div align="left" style="padding: 3px">
							<input type="hidden" id="curPageNO" name="curPageNO"
								value="${curPageNO}" /> 栏目名称 <input type="text"
								name="newsCategoryName" maxlength="50"
								value="${bean.newsCategoryName}" /> 状态 <select id="status"
								name="status">
               ...
               <input type="text" name="userName" maxlength="50"
							...
							<auth:auth ifAnyGranted="FA_SHOP">
								<input type="button" value="创建栏目"
									onclick='window.location="${pageContext.request.contextPath}/admin/newsCategory/load"' />
							</auth:auth>
							<input type="button" value="新闻列表"
								onclick='window.location="${pageContext.request.contextPath}/admin/news/query/1"' />
						</div>
					</td>
				</tr>
			</tbody>
		...
		<display:table name="list" requestURI="/admin/newsCategory/query"
			...
			<auth:auth ifAnyGranted="FA_SHOP">
				<display:column title="操作" media="html" style="width:40px">
					<a
						href="${pageContext.request.contextPath}/admin/newsCategory/load/${item.newsCategoryId}"
						title="修改"><img alt="修改"
						src="<ls:templateResource item='/common/default/images/grid_edit.png'/> "></a>
					<a href='javascript:deleteById("${item.newsCategoryId}")'
						title="删除"><img alt="删除"
	...
	<table style="width: 100%; border: 0px">
		<tr>
			<td align="left">说明：<br> 1. 每个文章都是挂在栏目下面，是文章的分类<br> 2.
				栏目处于上线状态页面可见，处于下线状态页面不可见<br>
			</td>
		<tr>
	</table>
