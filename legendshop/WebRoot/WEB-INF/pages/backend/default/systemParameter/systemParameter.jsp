...
<body>
	<form
		action="${pageContext.request.contextPath}/admin/system/systemParameter/save"
		...
		<div align="center">
			<table align="center" class="${tableclass}" id="col1">
				<thead>
					<tr class="sortable">
						<th colspan="2">
							<div align="center">${bean.name} 参数</div>
						</th>
					</tr>
				</thead>
				<tr>
				...
				<tr>
					...
					<td><c:choose>
							<c:when
								test="${bean.type == 'Selection' || bean.type == 'Boolean'}">
							...
							<c:when test="${bean.type == 'Password'}">
								<input type="password" name="value" id="value" size="50"
									maxlength="200" />
							</c:when>
							<c:otherwise>
								<input type="text" name="value" id="value" value="${bean.value}"
					...
					<td colspan="2">
						<div align="center">
							...
							<input type="reset" value="重置" /> <input type="button"
								value="返回" onclick="window.location='javascript:history.go(-1)'" />
						</div>
					</td>
				</tr>
			</table>
