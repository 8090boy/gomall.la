...
<body>
    ...
    %>
	<form action="${pageContext.request.contextPath}/admin/imgFile/save"
		...
		<table class="${tableclass}" style="width: 100%">
			...
			<tr>
				<td>商品图片 <font color="ff0000">*</font> <input type="file"
					name="file" id="file" size="30" /> <input type="hidden"
					name="productId" id="productId" value="${prod.prodId}" /><input
	...
	<form action="${pageContext.request.contextPath}/admin/imgFile/query"
		id="queryImgForm" method="post">
	...
	<div align="center">
		...
		<display:table name="list" requestURI="/admin/imgFile/query" id="item"
			...
			<display:column title="文件">
				<a href="<ls:photo item='${item.filePath}'/>" target="_blank"> <img
					src="<ls:images item='${item.filePath}'/>" height="150px"
					width="150px" /></a>
			</display:column>
			...
			<display:column title="上传时间">
				<fmt:formatDate value="${item.upoadTime}" pattern="yyyy-MM-dd HH:mm" />
			</display:column>
			<display:column title="操作" media="html" style="width:50px">
				<auth:auth ifAnyGranted="FA_PROD">
					<c:choose>
						<c:when test="${item.status == 1}">
							<img name="statusImg" itemId="${item.fileId}" itemName="图片"
								status="${item.status}" class="cursor_pointer"
					...
					<a href='javascript:deleteById("${item.fileId}")' title="删除"><img
						alt="删除"
	...
	<table style="width: 100%; border: 0px">
		<tr>
			<td align="left">说明：<br> 1. 点击选择，再点击提交则为商品增加一个图片，大小不能超过2M<br>
				3. 支持上传的图片格式有<font color="red">.jpg,.gif,.png,.jpeg</font>,其余文件格式不支持<br>
				2. 图片处于上线状态则会在商品的详细信息页面出现，处于下线状态则不会在页面上显示<br> 3. 图片删除则不会再显示<br>
			</td>
		<tr>
	</table>
