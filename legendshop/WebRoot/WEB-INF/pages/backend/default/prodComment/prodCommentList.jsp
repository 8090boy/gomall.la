...
<body>
    ...
    %>
	<form
		action="${pageContext.request.contextPath}/admin/productcomment/query"
			...
			<thead>
				<tr>
					<th><a href="<ls:url address='/admin/index'/>"
						...
						href="${pageContext.request.contextPath}/admin/productcomment/query">评论管理</a></th>
				</tr>
			</thead>
			<tbody>
				<tr>
					<td>
						<div align="left" style="padding: 3px">
							<input type="hidden" id="curPageNO" name="curPageNO"
								value="${curPageNO}" /> 评价人&nbsp;<input type="text"
								name="userName" id="userName" maxlength="50"
								value="${bean.userName}" /> 商品&nbsp;<input type="text"
								name="prodName" id="prodName" maxlength="50"
								value="${bean.prodName}" />
							<auth:auth ifAnyGranted="F_VIEW_ALL_DATA">
            	商城&nbsp;<input type="text" name="ownerName" maxlength="50"
	...
	<div align="center">
		...
		<display:table name="list" requestURI="/admin/productcomment/query"
			...
			<display:column title="商品" sortable="true" sortName="prod.name">
				<a target="_blank"
					href="${pageContext.request.contextPath}/views/${item.prodId}">${item.prodName}</a>
			</display:column>
			<display:column title="评论内容" property="content"></display:column>
			<display:column title="评论人" property="userName"></display:column>
			<display:column title="评论时间" sortable="true" sortName="hc.addtime">
			...
			<display:column title="操作" media="html" class="actionwidth">
				<a
					href="${pageContext.request.contextPath}/admin/productcomment/load/${item.id}"
					title="回复"><img alt="回复"
	...
	<table style="width: 100%; border: 0px">
		<tr>
			<td align="left">说明：<br> 1. 用户在前台录入商品评论<br> 2.
				可以回复商品评论或者删除不合理的评论<br>
			</td>
		<tr>
	</table>
