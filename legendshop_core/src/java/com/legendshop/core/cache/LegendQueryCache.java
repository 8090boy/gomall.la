...
public class LegendQueryCache implements Cache {
	...
	public void put(Object key, Object value) {
		if (this.getName().endsWith(SUFFIX) && cacheManager.isSupportQueryCache()
				&& Collection.class.isAssignableFrom(value.getClass())) {
			// for list
			// 如果是列表，则保存以ID为主键的关系
			...
			for (Object entityObj : coll) {
				...
				// put relevant into relcache format: entity name +
				// id:{[cacheName, key]}
				Cache relCache = cacheManager.getCache(cacheManager.getRelCacheName());
				String relCacheKey = entityCacheName + entity.getId();
				IdListRel valueWrapper = (IdListRel) relCache.get(relCacheKey);
				if (valueWrapper == null) {
					valueWrapper = new IdListRel(entity.getId());
				}
