...
public class EMFMain {
	...
	public static void main(String[] args) {
		...
		try {
			...
			Runnable server = (Runnable) dLoader.loadClass("com.legendshop.test.EMFServer").newInstance();

			Thread t = new Thread(server, "EMFServer");
			t.setContextClassLoader(dLoader);
			System.out.println("ClassLoader server: " + t.getContextClassLoader());
			t.start();
			t.join();
		} catch (Exception e) {
			System.err.println("An unrecoverable exception occured trying to start the EMFServerThread: " + e);
	...
	private static void loadProperties() {
		try {
			File props = new File(emfHome.getAbsoluteFile() + System.getProperty("file.separator", "/") + "conf");
			...
			System.out.println("An error has occured loading the properties: " + e);
			System.exit(-1);
		}
	}
	...
	private static void loadLibraries() {
		try {
			File libs = new File(emfHome.getAbsolutePath() + System.getProperty("file.separator", "/") + "lib");
			FileFilter filter = new FileFilter() {
				public boolean accept(File file) {
					String p = file.getAbsolutePath();
			...
			System.err.println("An error has occured trying to load the libraries: " + e);
			System.exit(-1);
		}
	}
}